...
export class AprComponent implements OnInit {
   ...
   */
  currentWeek =
    this.weekFromEaster < 8
      ? ` по Пасхе ${this.weekFromEaster}`
      : ` по Пятьдесятнице: ${this.weekFromEaster - 7}`;
  ...
  constructor(private timeBox: TimeboxService) {
    try {
      this.date =
        this.timeBox.theMoment.getDate() +
        ' ' +
        this.timeBox.formatsEaster.currentMonth;
      this.yearEaster = this.timeBox.theMoment.getFullYear() + '';
      // Получение aprakos.json с сервера.
      fetch(this.url)
        .then((loadedJson) => loadedJson.text())
        .then((loadedJsonText) =>
          this.currentZachalo(JSON.parse(loadedJsonText))
        )
        .catch(() => alert('Неудачная загрузка. Обновите страницу…'));
    } finally {
      // this.apstl = "Загрузка..."
      this.evngl = 'Загрузка...';
    }
  ...
  currentZachalo(json: any) {
    ...
    for (const key in json) {
      const curApr =
        '' +
        ((this.timeBox.formatsEaster.currentWeek as number) - this.stupka) +
        this.timeBox.formatsEaster.dayNum;

...
import { MyType } from 'src/app/mytype';
// import "./date-csl.ts";
 ...
 *
 */
export class TimeboxService implements OnInit {
  ...
  *
  */
  CONST_MLS_MiF = this.CONST_MLS_DAY * 7 * 10


  CONST_LOG_WARNING = "Будте вниматильней, проверте правильность вводимой даты."
  ER_606 = "НЕ ОПРЕДЕЛЕН КЛЮЧ ГОДА"
  ...
  */
  mif(): string {
    ...
    this.formatsEaster.mifMLS = (this.formatsEaster.nextEasterMLS as number) - this.CONST_MLS_MiF
  ...
  voznesnieGospodne(): string {
    ...
    this.formatsEaster.voznesenieMLS = (this.formatsEaster.lastEasterMLS as number) + (this.CONST_MLS_DAY * 39)
  ...
  pyatDesyatnica(): string {
    ...
    this.formatsEaster.pyatiDesyatnicaMLS = (this.formatsEaster.lastEasterMLS as number) + (this.CONST_MLS_DAY * 49)
  ...
  calculateAllWeks(): string {
    ...
    // четыредесятницы)
    this.formatsEaster.beginningLentMLS = parseInt((this.formatsEaster.nextEasterMLS as number - this.CONST_MLS_DAY * 48) + "", 10)
    ...
    let zero = parseInt((this.formatsEaster.nextEasterMLS as number - (this.formatsEaster.momentMLS as number)) / this.CONST_MLS_DAY + "", 10)
...
// Функция должна вычислять размер отступки до наступления Воздвиженского Понедельника, которая корректирует ссылку на Апракос в том случае, если наступил понедельник 18 седмицы, а праздника Воздвижения еще не случилось.
  ...
  vhodGospoden(): string {
    ...
    // if(this.keySystemYear != 0){
    this.formatsEaster.vhodMLS = (this.formatsEaster.nextEasterMLS as number) - this.CONST_MLS_DAY * 7
